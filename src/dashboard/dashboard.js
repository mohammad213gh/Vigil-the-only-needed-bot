...
function renderCommands(data){const el=document.getElementById('cmdList');const totals=data.reduce((a,c)=>a+c.commands.length,0);el.innerHTML='<div style="margin-bottom:14px;font-size:11px;color:var(--text-dim);">'+totals+' commands across '+data.length+' categories</div>'+data.map(cat=>'<div class="cmd-cat" onclick="this.classList.toggle(\'collapsed\')"><div class="cmd-cat-h"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><polyline points="6 9 12 15 18 9"/></svg><span>'+cat.category+'</span><span class="cmd-count">'+cat.commands.length+'</span><span class="cmd-bdg '+(cat.owner?'owner':'public')+'">'+(cat.owner?'Owner Only':'Public')+'</span></div><div class="cmd-items">'+cat.commands.map(cmd=>'<div class="cmd-item"><code class="cmd-name">/'+cmd.name+'</code><div class="cmd-desc">'+cmd.description+'</div><div class="cmd-usage"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><polyline points="4 17 10 11 4 5"/><line x1="12" y1="19" x2="20" y2="19"/></svg>'+cmd.usage+'</div></div>').join('')+'</div></div>').join('')}
...
checkAuth().then(async ok=>{if(!ok)return;initThemes();initBgStyles();await loadCfg();startBg(cfg.backgroundStyle||'dots');await loadOv();await loadAn();await loadServers();await loadAct();await loadCommands();await loadRm();await loadSys();loadBrand();stRf();initSSE()});
