    ...
    try {
        ...
        var html = '<div class="card" style="margin-bottom:16px;padding:16px;">' +
            ...
            (presence ? '<div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;">' +
                ...
                '</div>' +
                '<button class="btn btn-s" data-fn="leaveVoicePresence" data-args=\'['+fnData(serverId)+']\'>Disconnect</button>' +
                '<button class="btn btn-s" data-fn="moveVoicePresenceUI" data-args=\'['+fnData(serverId)+']\'>Move Channel</button>' +
                '</div>' :
                ...
                '<button class="btn btn-s" data-fn="joinVoicePresenceUI" data-args=\'['+fnData(serverId)+']\'>Join Channel</button>' +
                '</div>') + '</div>';
        ...
        html += '<div class="card" style="margin-bottom:16px;padding:16px;">' +
            ...
            '<button class="btn btn-s" data-fn="setVoicePresenceStatus" data-args=\'['+fnData(serverId)+']\'>Update Status</button>' +
            '</div>' +
        ...
        if (presence) {
            html += '<div class="card" style="padding:16px;">' +
                ...
                '<button class="btn btn-s" data-fn="moveVoicePresence" data-args=\'['+fnData(serverId)+']\'>Move</button>' +
                '</div></div>';
        }
...
export async function joinVoicePresenceUI(serverId) {
    ...
    try {
        ...
        var html = '<div class="stg" style="margin-bottom:16px;"><label>Join Voice Channel</label>' +
            ...
            '<button class="btn btn-s" data-fn="joinVoicePresence" data-args=\'['+fnData(serverId)+']\'>Join</button>' +
            '</div></div>' +
            '<button class="btn btn-s" data-fn="loadVoicePresence">Back</button>';
        cont.innerHTML = html;
            ...
            <div class="card" style="margin-bottom:12px;">
                <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;">
                    ...
                    <div style="display:flex;gap:8px;flex-wrap:wrap;">
                        <button class="btn btn-s" data-fn="copyToClipboard" data-args='[${fnData(esc(w.url))}]'>Copy URL</button>
                        <button class="btn btn-s" style="background:var(--danger)" data-fn="deleteWebhook" data-args='[${fnData(srv)},${fnData(w.id)}]'>Delete</button>
                    </div>
                </div>
            </div>
        `).join('');
    ...
    try {
        ...
        cont.innerHTML = tokens.map(t => `
            <div class="card" style="margin-bottom:12px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px;">
                ...
                <button class="btn btn-s" style="background:var(--danger)" data-fn="deleteApiToken" data-args=\'['+fnData(t.id)+']\'>Revoke</button>
            </div>
        `).join('');
