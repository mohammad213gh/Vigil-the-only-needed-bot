...
export async function loadOv(){
  try{
    ...
    if(s.online){
      // Derive a clean status label + dot color from the raw WS status
      // (backend strings may carry legacy emoji prefixes — strip them).
      var stRaw=String(s.status||'Ready').replace(/[^\w \-]/g,'').trim();
      var stTxt=stRaw||'Ready',stCls='g';
      if(/reconnect|connecting/i.test(stTxt)){stTxt='Connecting';stCls='y'}
      else if(/disconnect|nearly/i.test(stTxt)){stTxt='Disconnected';stCls='r'}
      else if(/idle/i.test(stTxt)){stTxt='Idle';stCls='y'}
      else{stTxt='Ready'}
      ...
      document.getElementById('ovCards').innerHTML=
        '<div class="card sr"><svg class="ico-bg" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M22 12h-4l-3 9L9 3l-3 9H2"/></svg><div class="lbl">Connection</div><div class="val"><span class="ovc-conn"><span class="st-dot '+(stCls||'g')+'"></span>'+(stTxt||'Ready')+'</span></div><div class="sub">'+s.ping+'ms ping</div></div>'+
...
var maxJoin=Math.max.apply(null,tl.map(function(s){return s.joins||0}));var maxLeave=Math.max.apply(null,tl.map(function(s){return s.leaves||0}));var overallMax=Math.max(maxJoin,maxLeave,1);var anW=Math.max(100,tl.length*8+40),anH=200,p=14,ch2=160,cw2=anW-p*2;var joinPts=tl.map(function(s,i){var x=p+(i/(tl.length-1||1))*cw2,y=anH-24-(s.joins||0)/overallMax*(ch2-8);return x+','+y;}).join(' ');var leavePts=tl.map(function(s,i){var x=p+(i/(tl.length-1||1))*cw2,y=anH-24-(s.leaves||0)/overallMax*(ch2-8);return x+','+y;}).join(' ');var joinArea=tl.map(function(s,i){var x=p+(i/(tl.length-1||1))*cw2,y=anH-24-(s.joins||0)/overallMax*(ch2-8);return x+','+y;}).join(' ')+' '+p+','+(anH-24)+' '+(p+cw2)+','+(anH-24);var leaveArea=tl.map(function(s,i){var x=p+(i/(tl.length-1||1))*cw2,y=anH-24-(s.leaves||0)/overallMax*(ch2-8);return x+','+y;}).join(' ')+' '+p+','+(anH-24)+' '+(p+cw2)+','+(anH-24);const anDots=tl.map(function(s,i){var x=p+(i/(tl.length-1||1))*cw2,jy=anH-24-(s.joins||0)/overallMax*(ch2-8),ly=anH-24-(s.leaves||0)/overallMax*(ch2-8);return '<circle cx="'+x+'" cy="'+jy+'" r="2.5" fill="#30d158" stroke="var(--bg)" stroke-width="1.5"><title>'+s.date+': +'+(s.joins||0)+'</title></circle>'+'<circle cx="'+x+'" cy="'+ly+'" r="2.5" fill="#ff453a" stroke="var(--bg)" stroke-width="1.5"><title>'+s.date+': -'+(s.leaves||0)+'</title></circle>';}).join('');var anLabels=tl.map(function(s,i){if(i%Math.max(1,Math.floor(tl.length/6))===0||i===tl.length-1){var x=p+(i/(tl.length-1||1))*cw2;return'<text x="'+x+'" y="'+(anH-6)+'" text-anchor="middle" fill="var(--text-muted)" font-size="8" font-family="Inter,sans-serif">'+s.date.slice(5)+'</text>'}return''}).join('');document.getElementById('growthChart').innerHTML='<svg viewBox="0 0 '+anW+' '+anH+'" preserveAspectRatio="none" style="width:100%;height:190px;display:block;"><defs><linearGradient id="jgF2" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="#3ba55c" stop-opacity="0.2"/><stop offset="100%" stop-color="#3ba55c" stop-opacity="0.01"/></linearGradient><linearGradient id="lgF2" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="#ed4245" stop-opacity="0.12"/><stop offset="100%" stop-color="#ed4245" stop-opacity="0.01"/></linearGradient></defs><polygon points="'+joinArea+'" fill="url(#jgF2)"/><polygon points="'+leaveArea+'" fill="url(#lgF2)"/><path d="'+smoothPath(joinPts)+'" fill="none" stroke="#30d158" stroke-width="2" stroke-linecap="round"/><path d="'+smoothPath(leavePts)+'" fill="none" stroke="#ff453a" stroke-width="1.5" stroke-linecap="round" opacity="0.65"/><text x="10" y="14" fill="#3ba55c" font-size="8" font-family="Inter,sans-serif" opacity="0.75">Joins</text><text x="10" y="25" fill="#ed4245" font-size="8" font-family="Inter,sans-serif" opacity="0.75">Leaves</text>'+anDots+anLabels+'</svg>'}
